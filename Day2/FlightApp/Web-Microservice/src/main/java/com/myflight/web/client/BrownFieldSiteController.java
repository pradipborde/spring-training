package com.myflight.web.client;
...
public class BrownFieldSiteController {
	...
	@Bean
	@LoadBalanced
	RestTemplate getSearchTemplate() {
		return new RestTemplate();
	}
	
	@RequestMapping(value = "/", method = RequestMethod.GET)
	public String greetingForm(Model model) {
		SearchQuery query = new SearchQuery("BOM", "BLR", "22-JAN-18");
	...
	public String greetingSubmit(@ModelAttribute UIData uiData, Model model) {
		...
		Flight[] flights = restClient.postForObject("http://search-apigateway/search/get", uiData.getSearchQuery(),
				Flight[].class);
	...
	public String ConfirmBooking(@ModelAttribute UIData uiData, Model model) {
		...
		try {
			...
			// booking, long.class);
			bookingId = restClient.postForObject("http://booking-service/booking/create", booking, long.class);
	...
	public String searchBookingSubmit(@ModelAttribute UIData uiData, Model model) {
		...
		BookingRecord booking = restClient.getForObject("http://booking-service/booking/get/" + id,
				BookingRecord.class);
